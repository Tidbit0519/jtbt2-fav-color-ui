...
terraform {
  backend "s3" {
    ...
    key            = "jtbt2-fav-color-ui/prd/app.tfstate"   // TODO replace <APP_NAME>
    region         = "us-west-2"
  }
  required_providers {
    aws = {
...
locals {
  app_name = "jtbt2-fav-color-ui"        // TODO replace <APP_NAME>
  url      = "${local.app_name}.byu.edu" // TODO double check if <APP_NAME>.byu.edu is what you want for your public URL
  default_tags = {
    repo             = "https://github.com/byu-oit/jtbt-fav-color-ui" # TODO fix to match your GitHub repo
    app              = local.app_name
